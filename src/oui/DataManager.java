...
public class DataManager {
	...
	public static void CreateData(int numRows, DataPanel dp) throws Exception {
		...
		for (int i = 1; i <= numRows; i++) {
			...
			if (canThePageAccomodateARow(pagePath, row)) {	// 8 KB bhar geya?
				...
				if (pageNumber == 9) {
					...
					dp.updateStatus((i * 100) / numRows);	// every extent ke baad status update krdiya
				}
			}
		}
			...
			try {
				...
				do {
					...
					if (line != null && line.trim().length() > 0) {
						...
						addLineToIndex(value, address, specificIndexDirPath);	// addvalue to Btree
					}
			...
			while (rowsCounter <= rowsTotal) {
				...
				try {
					...
					do {
						...
						if (line != null && line.trim().length() > 0) {	// jab tk page khatm na hoje
							String value = line.split("[|]")[columnIndex];	// [id, name, username, pswd]
	...
	private static String getRandomWord() {
		int length = (int) (Math.random() * 10) + 5;	// length will be 5 to 15 
 ...
 * @param indexDirPath
 * @param node
 * @param nodeLeafMap tells if leaf / non-leaf page (will be found from the meta data)
 * @param columnValue
 * @param start
 * @param result
 * @param qp
		...
		if (!isLeaf) {
			...
			try {
				...
				if (flag) {	//mil gya
					...
					// larger
					childNode = prevLine.split("=")[1].split("[|]")[1];	//akhri line ka right hissa
				}
